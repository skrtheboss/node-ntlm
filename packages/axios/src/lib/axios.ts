    ...
    client.interceptors.response.use(
        ...
        async (err: AxiosError) => {
            ...
            if (error && error.status === 401) {
                const ntlmMessage = extractNtlmMessageFromAuthenticateHeader(error.headers['www-authenticate']);
                ...
                if (ntlmMessage) {
                    ...
                    if (ntlmMessage.length < 50) {
                        ...
                        const t3Msg = createType3Message(parseType2Message(ntlmMessage), safeCredentials);
